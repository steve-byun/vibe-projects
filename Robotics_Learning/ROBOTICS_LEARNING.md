...
| `s10_trajectory_generation.m` | trajectory | 궤적 생성, MoveJ/L/C, 속도 프로파일 | ✅ |
...
### 260211 - 궤적 생성 (Trajectory Generation)

#### s10: MoveJ vs MoveL vs MoveC
- **핵심**: 같은 A→B인데 "중간 경로"가 완전히 다르다
- MoveJ (관절 공간): 관절 직선 보간 → 끝점은 예측 불가 곡선
  - 관절 부드럽게, 끝점 속도 울퉁불퉁
  - 용도: 빈 공간 이동, 홈 복귀 (경로 상관없을 때)
- MoveL (작업 공간 직선): 끝점 직선 보간 + 매번 IK
  - 끝점 직선, 관절은 비선형으로 움직임
  - 용도: 용접, 절단, 조립 (직선 경로 필요)
- MoveC (작업 공간 원호): 끝점이 정확한 원호를 따라감
  - 용도: 나사 조이기, 원형 연마, 디버링
...
- 5차: 전부 부드러움 (학습용)
- 사다리꼴: 가속→등속→감속 (실무 표준)
  - 전환점에서 가속도 점프 → 약간 충격 (jerk)
  - S-커브로 보완 가능 (꺾이는 부분 둥글게)

#### 핵심 이해 과정
- "MoveJ 끝점 속도가 울퉁불퉁하면?" → 용접 비드 불균일 (느린 구간 두꺼움, 빠른 구간 얇음)
- "사다리꼴 가속도가 0이 아닌데?" → 크기가 아니라 "갑자기 바뀜"이 문제 (충격/저크)
- "MoveJ도 곡선인데 MoveC 대신 안 됨?" → MoveJ는 예측 불가 곡선, MoveC는 정확한 원호
- 실무 패턴: MoveJ로 이동 → MoveL/MoveC로 정밀 작업
...
**마지막 업데이트**: 2026-02-10 (s08 CT, s09 임피던스 제어 학습 완료, 2-Link 기초 전 과정 완료!)
