...
while (ishandle(f))
    ...
    fa = [p.Fx; p.Fy; 0; 0; 0; 0];
    p_d = [p.xtarget; p.ytarget; 0; 0; 0; 0];
    dotp_d = zeros(6,1);
    ddotp_d = zeros(6,1);
    [zdot1, T1, T2] = p.ode_fn(tnew, z1, fa, p_d, dotp_d, ddotp_d, p, p.ctrl_law);

    % [zdot1, T1, T2] = FullDyn(tnew,z1,p); -> original code
