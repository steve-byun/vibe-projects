...
const REFRESH_MS = 15000; // 15초마다 파일 확인
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("claudeUsage.showDetails", showDetails));
    // 파일 감시 (실시간 반영)
    try {
...
function formatTimeRemaining_reverse(timestamp) {
...
function updateStatusBar() {
    ...
    statusBarItem.tooltip = new vscode.MarkdownString(lines.filter(Boolean).join("\n\n"));
}
function showDetails() {
    const data = readUsageData();
    if (!data || data.status !== "ok") {
        vscode.window.showWarningMessage("Claude Usage 데이터를 가져올 수 없습니다. Bridge 스크립트가 실행 중인지 확인하세요.");
        return;
    }
    const fiveHour = data.five_hour;
    const sevenDay = data.seven_day;
    ...
    const sevenDayItem = sevenDay
        ? sevenDay.resets_at
            ...
            : `7일 한도 : ${Math.round(sevenDay.utilization)}% 사용`
        : "";
    const items = [fiveHourItem, sevenDayItem].filter(Boolean);
